<template>
    <div class="relative w-full h-full group translate-y-0 lg:translate-y-12">
        <!-- Re-rendering Swiper on language change via :key ensures proper RTL initialization -->
        <swiper :key="locale" :dir="dir" :modules="[Autoplay, Navigation]" :slides-per-view="'auto'"
            :centered-slides="true" :loop="true" :autoplay="{ delay: 5000, disableOnInteraction: false }" :navigation="{
                prevEl: '.swiper-button-prev-custom',
                nextEl: '.swiper-button-next-custom',
            }" :space-between="0" class="w-full h-full overflow-visible! [&_.swiper-slide]:opacity-40 [&_.swiper-slide]:transition-all [&_.swiper-slide]:duration-700 [&_.swiper-slide-active]:opacity-100 
            [&_.swiper-slide_.product-container]:scale-[0.5] md:[&_.swiper-slide_.product-container]:scale-[0.6] 
            [&_.swiper-slide-active_.product-container]:scale-100 
            ...
            <!-- Navigation Buttons (Mirrored in RTL) -->
            <div
            ...
            <swiper-slide v-for="(product, index) in products" :key="index" class="w-[300px] md:w-[450px]!">
                <AnimatedContent direction="vertical" :distance="50" :delay="index * 0.1"
                    ...
                    <div
                        ...
                        <!-- Background Circle -->
                        <div class="absolute w-[240px] h-[240px] md:w-[320px] md:h-[320px] lg:w-[480px] lg:h-[480px] rounded-full z-0 transition-transform duration-700 bg-circle"
